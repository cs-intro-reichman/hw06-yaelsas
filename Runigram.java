...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		print(imageOut);
		// Tests the blending of 2 colors:
		System.out.println();
		Color c1 = new Color(255, 255, 255);
		Color c2 = new Color(0, 0, 0);
		print(blend(c1, c2, 0.75));
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < flippedImage.length; i++) {
			...
			for (int j = 0; j < flippedImage[0].length && k >= 0; j++) {
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int j = 0; j < flippedImage[0].length; j++) {
			...
			for (int i = 0; i < flippedImage.length && k >= 0; i++) {
				flippedImage[i][j] = image[k][j];
				k--;
			}
		}
		return flippedImage;
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				scaledImage[i][j] = image[(int)((i * orgHeight) / height)][(int)((j *orgWidth) / width)];
			}
		}
		return scaledImage;
	}
